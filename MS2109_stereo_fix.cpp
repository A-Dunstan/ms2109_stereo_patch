...
*  Byte 10: more bitflags, bit 0 seems to be something to do with HID...
*  Byte 11: ??
...
*  fixed offset in XDATA). When R7==0 the patch modifies the device descriptor by incrementing the bcdDevice value - this helps windows realize the device configuration has changed,
*  to prevent it from ignoring the actual device descriptors and using cached copies from the registry.
